...
# DeepSeek V4 thinking effort mapping (2 levels: high, max)
DEEPSEEK_V4_EFFORT = {
...
class DeepSeekProvider(ProviderInterface):
    ...
    def _convert_messages(self, messages: List[AgentMessage], system_prompt: str) -> List[Dict]:
        """Convert to OpenAI-compatible format, including tool_calls from history."""
        converted = []
    ...
    async def chat(
        ...
        # DeepSeek thinking: V4 supports thinking toggle + effort
        is_v4 = "v4" in model.model_id
        if is_v4 and model.supports_thinking and thinking_level != ThinkingLevel.OFF:
            thinking_config: Dict[str, Any] = {"type": "enabled"}
            effort = DEEPSEEK_V4_EFFORT.get(thinking_level, "high")
        ...
        # DeepSeek returns reasoning_content when thinking is active (V3 with thinking, or R1)
        thinking_content = msg_data.get("reasoning_content", "")
        if thinking_content:
            result.metadata["thinking"] = thinking_content

        if msg_data.get("content"):
            result.content = msg_data["content"]
            ...
            async for line in response.aiter_lines():
                ...
                if line.startswith("data: "):
                    ...
                    try:
                        ...
                        # DeepSeek returns reasoning_content for thinking output
